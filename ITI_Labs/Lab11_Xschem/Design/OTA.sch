v {xschem version=3.4.6 file_version=1.2}
G {}
K {}
V {}
S {
}
E {}
B 4 -490 -700 420 200 {fill =0}
B 4 430 -700 1770 200 {fill =0}
T {BIASING CIRCUIT} -490 -740 0 0 0.4 0.4 {}
T {FOLDED CASCODE OTA} 680 -740 0 1 0.4 0.4 {}
N 540 -600 540 -550 {lab=AVDD}
N 540 -600 1620 -600 {lab=AVDD}
N 1620 -600 1620 -550 {lab=AVDD}
N 800 -600 800 -550 {lab=AVDD}
N 1050 -600 1050 -550 {lab=AVDD}
N 800 -520 830 -520 {lab=AVDD}
N 830 -600 830 -520 {lab=AVDD}
N 1010 -520 1050 -520 {lab=AVDD}
N 1010 -600 1010 -520 {lab=AVDD}
N 1620 -520 1660 -520 {lab=AVDD}
N 1660 -600 1660 -520 {lab=AVDD}
N 1620 -600 1660 -600 {lab=AVDD}
N 540 -470 590 -470 {lab=IBn}
N 590 -520 590 -470 {lab=IBn}
N 510 -520 540 -520 {lab=AVDD}
N 510 -600 510 -520 {lab=AVDD}
N 510 -600 540 -600 {lab=AVDD}
N 1090 -520 1140 -520 {lab=IBn}
N 1520 -520 1580 -520 {lab=IBn}
N 1330 -600 1330 -550 {lab=AVDD}
N 1330 -520 1360 -520 {lab=AVDD}
N 1360 -600 1360 -520 {lab=AVDD}
N 800 30 800 80 {lab=AGND}
N 800 80 1620 80 {lab=AGND}
N 1620 30 1620 80 {lab=AGND}
N 1050 30 1050 80 {lab=AGND}
N 1090 0 1580 0 {lab=VBN}
N 840 0 890 0 {lab=VBN}
N 1050 -490 1050 -370 {lab=#net1}
N 1050 -310 1050 -170 {lab=VOUTN}
N 1050 -110 1050 -30 {lab=VDS1}
N 1620 -490 1620 -370 {lab=#net2}
N 1620 -310 1620 -170 {lab=VOUTP}
N 1620 -110 1620 -30 {lab=VDS2}
N 1150 -230 1190 -230 {lab=VINN}
N 1480 -230 1510 -230 {lab=VINP}
N 1510 -230 1520 -230 {lab=VINP}
N 1330 -490 1330 -460 {lab=VDS}
N 1230 -460 1330 -460 {lab=VDS}
N 1330 -460 1440 -460 {lab=VDS}
N 1440 -460 1440 -260 {lab=VDS}
N 1230 -460 1230 -260 {lab=VDS}
N 1230 -230 1260 -230 {lab=VDS}
N 1260 -310 1260 -230 {lab=VDS}
N 1230 -310 1260 -310 {lab=VDS}
N 1400 -230 1440 -230 {lab=VDS}
N 1400 -310 1400 -230 {lab=VDS}
N 1400 -310 1440 -310 {lab=VDS}
N 1230 -200 1230 -60 {lab=VDS1}
N 1050 -60 1230 -60 {lab=VDS1}
N 1440 -200 1440 -70 {lab=VDS2}
N 1440 -70 1620 -70 {lab=VDS2}
N 1620 -140 1680 -140 {lab=AGND}
N 1620 -340 1680 -340 {lab=AVDD}
N 980 -340 1050 -340 {lab=AVDD}
N 990 -140 1050 -140 {lab=AGND}
N 1010 0 1050 0 {lab=AGND}
N 1010 0 1010 80 {lab=AGND}
N 1620 0 1660 0 {lab=AGND}
N 1660 0 1660 80 {lab=AGND}
N 1620 80 1660 80 {lab=AGND}
N 770 0 800 0 {lab=AGND}
N 770 0 770 80 {lab=AGND}
N 770 80 800 80 {lab=AGND}
N 1090 -340 1120 -340 {lab=VCASCP}
N 1550 -340 1580 -340 {lab=VCASCP}
N 1090 -140 1120 -140 {lab=VCASCN}
N 1550 -140 1580 -140 {lab=VCASCN}
N 800 -60 860 -60 {lab=VBN}
N 860 -60 860 0 {lab=VBN}
N 1260 -520 1290 -520 {lab=VCTRLP}
N 800 -490 800 -30 {lab=VBN}
N 580 -520 760 -520 {lab=IBn}
N 540 -490 540 -470 {lab=IBn}
N 540 -470 540 -260 {lab=IBn}
N -310 -600 -310 -550 {lab=AVDD}
N -50 -600 -50 -550 {lab=AVDD}
N -50 -520 -20 -520 {lab=AVDD}
N -20 -600 -20 -520 {lab=AVDD}
N -310 -470 -260 -470 {lab=VB1}
N -260 -520 -260 -470 {lab=VB1}
N -340 -520 -310 -520 {lab=AVDD}
N -340 -600 -340 -520 {lab=AVDD}
N -340 -600 -310 -600 {lab=AVDD}
N -270 -520 -90 -520 {lab=VB1}
N -310 -490 -310 -470 {lab=VB1}
N -310 -600 -20 -600 {lab=AVDD}
N 130 -600 130 -550 {lab=AVDD}
N -20 -600 130 -600 {lab=AVDD}
N 130 -520 180 -520 {lab=AVDD}
N 180 -600 180 -520 {lab=AVDD}
N 130 -600 180 -600 {lab=AVDD}
N -310 -470 -310 -420 {lab=VB1}
N -50 -470 -50 -420 {lab=#net3}
N -50 -390 -20 -390 {lab=AVDD}
N -20 -470 -20 -390 {lab=AVDD}
N -310 -340 -260 -340 {lab=VB2}
N -260 -390 -260 -340 {lab=VB2}
N -340 -390 -310 -390 {lab=AVDD}
N -340 -470 -340 -390 {lab=AVDD}
N -270 -390 -90 -390 {lab=VB2}
N -310 -360 -310 -340 {lab=VB2}
N 130 -470 130 -420 {lab=#net4}
N 130 -390 180 -390 {lab=AVDD}
N 180 -470 180 -390 {lab=AVDD}
N -340 -520 -340 -470 {lab=AVDD}
N 180 -520 180 -470 {lab=AVDD}
N 130 -490 130 -470 {lab=#net4}
N 40 -390 90 -390 {lab=VB2}
N 40 -520 90 -520 {lab=VB1}
N -20 -520 -20 -470 {lab=AVDD}
N -50 -490 -50 -470 {lab=#net3}
N -310 -340 -310 -170 {lab=VB2}
N -410 -140 -350 -140 {lab=VBN}
N 130 -110 130 -70 {lab=#net5}
N 130 -360 130 -170 {lab=VCASCP}
N -110 -140 -90 -140 {lab=VCASCN}
N -110 -210 -110 -140 {lab=VCASCN}
N -110 -210 -50 -210 {lab=VCASCN}
N -50 -360 -50 -170 {lab=VCASCN}
N -310 -110 -310 40 {lab=AGND}
N -310 40 130 40 {lab=AGND}
N 130 -10 130 40 {lab=AGND}
N 80 -40 130 -40 {lab=AGND}
N 80 -40 80 40 {lab=AGND}
N 80 -140 130 -140 {lab=AGND}
N 80 -140 80 -40 {lab=AGND}
N -50 -110 -50 40 {lab=AGND}
N -50 -140 -10 -140 {lab=AGND}
N -10 -140 -10 40 {lab=AGND}
N 170 -140 280 -140 {lab=VCASCN}
N 170 -40 280 -40 {lab=VCASCP}
N 210 -220 210 -40 {lab=VCASCP}
N 130 -220 210 -220 {lab=VCASCP}
N -310 -140 -270 -140 {lab=AGND}
N -270 -140 -270 40 {lab=AGND}
C {symbols/pmos_3p3.sym} 1460 -230 0 1 {name=M2
L=500n
W=26.5u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=pmos_3p3
spiceprefix=X
}
C {symbols/pmos_3p3.sym} 1210 -230 0 0 {name=M1
L=500n
W=26.5u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=pmos_3p3
spiceprefix=X
}
C {symbols/pmos_3p3.sym} 1310 -520 0 0 {name=M9
L=1u
W=39u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=pmos_3p3
spiceprefix=X
}
C {symbols/pmos_3p3.sym} 1600 -340 0 0 {name=M6
L=500n
W=10.68u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=pmos_3p3
spiceprefix=X
}
C {symbols/pmos_3p3.sym} 1600 -520 0 0 {name=M11
L=1u
W=39u/5
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=pmos_3p3
spiceprefix=X
}
C {symbols/nmos_3p3.sym} 1600 -140 0 0 {name=M4
L=500n
W=3u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=nmos_3p3
spiceprefix=X
}
C {symbols/nmos_3p3.sym} 1600 0 0 0 {name=M8
L=1u
W=6.37u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=nmos_3p3
spiceprefix=X
}
C {symbols/pmos_3p3.sym} 1070 -340 0 1 {name=M5
L=500n
W=10.68u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=pmos_3p3
spiceprefix=X
}
C {symbols/pmos_3p3.sym} 1070 -520 0 1 {name=M10
L=1u
W=39u/5
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=pmos_3p3
spiceprefix=X
}
C {symbols/nmos_3p3.sym} 1070 -140 0 1 {name=M3
L=500n
W=3u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=nmos_3p3
spiceprefix=X
}
C {symbols/nmos_3p3.sym} 1070 0 0 1 {name=M7
L=1u
W=6.37u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=nmos_3p3
spiceprefix=X
}
C {symbols/pmos_3p3.sym} 780 -520 0 0 {name=M12
L=1u
W=39*0.7u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=pmos_3p3
spiceprefix=X
}
C {symbols/nmos_3p3.sym} 820 0 0 1 {name=M13
L=1u
W=6.37u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=nmos_3p3
spiceprefix=X
}
C {symbols/pmos_3p3.sym} 560 -520 0 1 {name=M14
L=1u
W=39u/2
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=pmos_3p3
spiceprefix=X
}
C {devices/iopin.sym} 540 -260 1 0 {name=p5 lab=IBn}
C {devices/lab_pin.sym} 670 -520 1 0 {name=p6 sig_type=std_logic lab=IBn}
C {devices/lab_pin.sym} 1140 -520 1 0 {name=p7 sig_type=std_logic lab=IBn}
C {devices/lab_pin.sym} 1520 -520 1 0 {name=p8 sig_type=std_logic lab=IBn}
C {devices/lab_pin.sym} 1350 0 1 0 {name=p16 sig_type=std_logic lab=VBN}
C {devices/iopin.sym} 1190 -600 3 0 {name=p17 lab=AVDD}
C {devices/iopin.sym} 1210 80 1 0 {name=p18 lab=AGND}
C {devices/iopin.sym} 1150 -230 2 0 {name=p19 lab=VINN}
C {devices/iopin.sym} 1520 -230 0 0 {name=p20 lab=VINP}
C {devices/lab_pin.sym} 1120 -340 1 0 {name=p21 sig_type=std_logic lab=VCASCP}
C {devices/lab_pin.sym} 1120 -140 1 0 {name=p22 sig_type=std_logic lab=VCASCN}
C {devices/lab_pin.sym} 1550 -340 1 0 {name=p23 sig_type=std_logic lab=VCASCP}
C {devices/lab_pin.sym} 1550 -140 1 0 {name=p24 sig_type=std_logic lab=VCASCN}
C {devices/iopin.sym} 1050 -230 2 0 {name=p25 lab=VOUTN}
C {devices/iopin.sym} 1620 -230 0 0 {name=p26 lab=VOUTP}
C {devices/lab_pin.sym} 990 -140 0 0 {name=p27 sig_type=std_logic lab=AGND}
C {devices/lab_pin.sym} 1680 -140 2 0 {name=p28 sig_type=std_logic lab=AGND}
C {devices/lab_pin.sym} 980 -340 0 0 {name=p29 sig_type=std_logic lab=AVDD}
C {devices/lab_pin.sym} 1680 -340 2 0 {name=p30 sig_type=std_logic lab=AVDD}
C {devices/iopin.sym} 1260 -520 2 0 {name=p31 lab=VCTRLP}
C {devices/iopin.sym} 890 0 3 0 {name=p9 lab=VBN}
C {devices/lab_pin.sym} 280 -140 2 0 {name=p3 sig_type=std_logic lab=VCASCN}
C {devices/lab_pin.sym} 280 -40 2 0 {name=p4 sig_type=std_logic lab=VCASCP}
C {symbols/pmos_3p3.sym} -70 -520 0 0 {name=M15
L=1u
W=39u/2
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=pmos_3p3
spiceprefix=X
}
C {symbols/pmos_3p3.sym} -290 -520 0 1 {name=M16
L=1u
W=39u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=pmos_3p3
spiceprefix=X
}
C {devices/lab_pin.sym} -180 -520 1 0 {name=p1 sig_type=std_logic lab=VB1}
C {symbols/pmos_3p3.sym} 110 -520 0 0 {name=M17
L=1u
W=39u/2
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=pmos_3p3
spiceprefix=X
}
C {symbols/pmos_3p3.sym} -70 -390 0 0 {name=M18
L=500n
W=10.68u/2
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=pmos_3p3
spiceprefix=X
}
C {symbols/pmos_3p3.sym} -290 -390 0 1 {name=M19
L=500n
W=10.68u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=pmos_3p3
spiceprefix=X
}
C {devices/lab_pin.sym} -180 -390 1 0 {name=p2 sig_type=std_logic lab=VB2}
C {symbols/pmos_3p3.sym} 110 -390 0 0 {name=M20
L=500n
W=10.68u/2
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=pmos_3p3
spiceprefix=X
}
C {devices/lab_pin.sym} 50 -520 1 0 {name=p10 sig_type=std_logic lab=VB1}
C {devices/lab_pin.sym} 50 -390 1 0 {name=p11 sig_type=std_logic lab=VB2}
C {symbols/nmos_3p3.sym} -330 -140 0 0 {name=M21
L=1u
W=6.37u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=nmos_3p3
spiceprefix=X
}
C {devices/lab_pin.sym} -410 -140 1 0 {name=p12 sig_type=std_logic lab=VBN}
C {symbols/nmos_3p3.sym} -70 -140 0 0 {name=M22
L=500n
W=430n
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=nmos_3p3
spiceprefix=X
}
C {symbols/nmos_3p3.sym} 150 -140 0 1 {name=M23
L=500n
W=430n
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=nmos_3p3
spiceprefix=X
}
C {symbols/nmos_3p3.sym} 150 -40 0 1 {name=M24
L=1u
W=30u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=nmos_3p3
spiceprefix=X
}
C {devices/lab_pin.sym} -110 -210 0 0 {name=p13 sig_type=std_logic lab=VCASCN}
C {devices/lab_pin.sym} -100 -600 1 0 {name=p14 sig_type=std_logic lab=AVDD}
C {devices/lab_pin.sym} -110 40 3 0 {name=p15 sig_type=std_logic lab=AGND}
C {devices/lab_pin.sym} 1230 -400 2 0 {name=p32 sig_type=std_logic lab=VDS}
C {devices/lab_pin.sym} 1230 -80 2 0 {name=p34 sig_type=std_logic lab=VDS1}
C {devices/lab_pin.sym} 1440 -80 0 0 {name=p35 sig_type=std_logic lab=VDS2}
